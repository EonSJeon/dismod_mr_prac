...
def build_random_effects_matrix(input_data, model, root_area, parameters):
    ...
    valid_cols = [
        col for col in U.columns
        if (1 <= U[col].sum() < len(U)) or (col in keep_consts)
    ]
    U = U[valid_cols].copy()
...
def build_sigma_alpha(name, parameters, max_depth=5):
    """
    name, parameters를 받아
    ...
    for i in range(max_depth):
        effect = f'sigma_alpha_{name}_{i}'
...
def build_alpha(name, U, sigma_alpha, parameters, zero_re, hierarchy):
    """
...
def mean_covariate_model(name, mu, input_data, parameters, model, root_area, root_sex, root_year, zero_re: bool = True):
    ...
    sigma_alpha = build_sigma_alpha(name, parameters)
    alpha, const_alpha_sigma, alpha_potentials = build_alpha(
        name, U, sigma_alpha, parameters, zero_re, model.hierarchy
    )
    # === 고정 효과 행렬 X와 beta 생성 ===
        ...
        if len(covariates.columns) > 1:
            ...
            for cov in covariates:
                if cov != 'pop':
                    ...
                    X_shift[cov] = (leaf_covariates[cov] * leaf_covariates['pop']).sum() / leaf_covariates['pop'].sum()
